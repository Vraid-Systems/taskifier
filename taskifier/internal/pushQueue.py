...
def _create_task_from_db(db_Task_obj):
    ...
    queue_Task_obj = queue_Task(payload=None, name=None,
                                ...
                                headers={},
                                params={const.KEY_SOURCE: db_Task_obj.source,
                                        const.KEY_DEST: db_Task_obj.dest,
                                        const.KEY_CONTENT: db_Task_obj.content},
                                countdown=0)
...
def _get_ready_tasks():
    ...
    return db_Task.objects.filter(ready_time__lte=datetime.utcnow())
